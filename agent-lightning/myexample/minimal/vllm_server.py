...
def vllm_server(
    ...
    参数：
        model_path: vLLM 模型的路径。可以是本地路径或 Hugging Face 模型 ID。
        port: 服务模型的端口。
        startup_timeout: 服务器启动超时时间。
        terminate_timeout: 服务器终止超时时间。
        gpu_memory_utilization: 服务器的 GPU 内存利用率。设置较低可避免内存溢出。
        auto_tool_choice: 是否启用自动工具选择。
        tool_call_parser: 要使用的工具调用解析器。
    """
